...
const SupervisorCompleted = () => {
  ...
  const handleApprove = async (reportId) => {
    ...
    try {
      ...
      setReports((prev) =>
        prev.map((r) =>
          r._id === reportId ? { ...r, status: "Completed" } : r
        )
      );
    } catch (error) {
                    ...
                    <div className="flex-1">
                      ...
                      {report.status === "Completed" ? (
                        ...
                        <button
                          ...
                          className={`mt-2 px-3 py-1 rounded text-white text-xs font-semibold ${approving === report._id
                              ? "bg-gray-400 cursor-not-allowed"
                              : "bg-emerald-700 hover:bg-emerald-800"
                            }`}
                        >
                          {approving === report._id ? "Approving..." : "Approve Task"}
                        </button>
                      )}
